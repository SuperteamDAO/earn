  ...
  return (
    <>
      <div className="flex w-full items-start bg-white">
        <div className="flex min-h-[600px] w-full flex-col bg-white md:grid md:grid-cols-[23rem_1fr]">
          <div
            className={cn(
              'h-full w-full',
          ...
          <div
            className={cn(
              'h-full w-full rounded-r-xl border-t border-r border-b border-slate-200 bg-white',
              mobileView === 'list' ? 'hidden md:block' : 'block',
            )}
          >
            {mobileView === 'detail' && (
              <button
                className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-slate-500 hover:text-slate-700 md:hidden"
      ...
      <Dialog modal={false} onOpenChange={onTogglerClose} open={isTogglerOpen}>
        ...
        >
          <div className="mx-auto w-full rounded-lg sm:w-fit">
            ...
            <div className="flex max-w-full items-center justify-center gap-1 overflow-x-auto whitespace-nowrap pr-1 [&::-webkit-scrollbar]:hidden sm:justify-start sm:gap-2">
