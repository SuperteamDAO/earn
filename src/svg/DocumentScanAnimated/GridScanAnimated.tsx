...
function GridScanItem({ index, cycle }: { index: number; cycle: number }) {
  ...
  const { bg, stroke, border } = COLOR_PAIRS[colorIdx]!;
  ...
  return (
    <motion.div
      ...
      className="relative flex items-center justify-center overflow-hidden rounded-lg border-[0.09375rem] border-transparent p-2 transition-colors duration-700"
      style={{
        background: revealed ? bg : 'var(--color-slate-50)',
        transition: 'background 0.5s border 0.5s',
        borderColor: revealed ? border : 'transparent',
      }}
    >
      <Document
        className="!size-10"
...
export function GridScanAnimation() {
  ...
  return (
    <div className="relative mx-auto w-fit">
      ...
      <div
        ref={gridRef}
        className="mx-auto grid w-fit grid-cols-5 grid-rows-3 gap-5 overflow-hidden"
      >
        {grid.map((_, i) => (
