...
function getFlagUrl(code: string): string | null {
  ...
  if (CUSTOM_FLAGS[lowerCode]) {
    return CUSTOM_FLAGS[lowerCode];
  }
  if (code.length === 2) {
...
export async function GET(request: Request) {
  try {
    ...
    const allText = `${region || ''}${prefix}Remote web3 gigs for talent Explore Opportunities`;
    ...
    // Get flag URL (handles custom regions and standard country codes)
    const flagUrl = hasCode ? getFlagUrl(code) : null;

    if (!isSuperteam && hasCode) {
