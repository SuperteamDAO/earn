...
CREATE TABLE `BountiesTemplates` (
    ...
    `type` ENUM('bounty', 'project', 'hackathon') NOT NULL DEFAULT 'bounty',
    `requirements` TEXT NULL,
    `region` ENUM('GLOBAL', 'INDIA', 'VIETNAM', 'GERMANY', 'TURKEY', 'MEXICO', 'UK', 'UAE', 'NIGERIA', 'ISRAEL', 'BRAZIL', 'MALAYSIA', 'BALKAN', 'PHILIPPINES', 'JAPAN', 'FRANCE', 'CANADA', 'SINGAPORE', 'POLAND', 'KOREA', 'UKRAINE', 'ARGENTINA', 'USA', 'SPAIN') NOT NULL DEFAULT 'GLOBAL',
    `applicationType` ENUM('rolling', 'fixed') NOT NULL DEFAULT 'fixed',
    `status` ENUM('OPEN', 'REVIEW', 'CLOSED', 'VERIFYING', 'VERIFY_FAIL') NOT NULL DEFAULT 'OPEN',
    `timeToComplete` VARCHAR(191) NULL,
    `token` VARCHAR(191) NULL,
...
CREATE TABLE `Comment` (
    ...
    INDEX `Comment_refId_isActive_isArchived_type_idx`(`refId`, `isActive`, `isArchived`, `type`),
    INDEX `Comment_refId_isActive_isArchived_replyToId_idx`(`refId`, `isActive`, `isArchived`, `replyToId`),
    INDEX `Comment_refType_type_idx`(`refType`, `type`),
    INDEX `Comment_updatedAt_idx`(`updatedAt` DESC),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Grants` (
    ...
    INDEX `Grants_isPublished_isActive_isArchived_isPrivate_idx`(`isPublished`, `isActive`, `isArchived`, `isPrivate`),
    INDEX `Grants_isPublished_isActive_isArchived_status_idx`(`isPublished`, `isActive`, `isArchived`, `status`),
    INDEX `Grants_region_isPublished_isActive_idx`(`region`, `isPublished`, `isActive`),
    INDEX `Grants_slug_idx`(`slug`),
    INDEX `Grants_sponsorId_isActive_isArchived_status_idx`(`sponsorId`, `isActive`, `isArchived`, `status`),
    INDEX `Grants_createdAt_idx`(`createdAt` DESC),
    INDEX `Grants_totalApproved_idx`(`totalApproved` DESC),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `GrantApplication` (
    ...
    INDEX `GrantApplication_grantId_userId_applicationStatus_idx`(`grantId`, `userId`, `applicationStatus`),
    INDEX `GrantApplication_applicationStatus_decidedAt_idx`(`applicationStatus`, `decidedAt`),
    INDEX `GrantApplication_applicationStatus_createdAt_idx`(`applicationStatus`, `createdAt`),
    INDEX `GrantApplication_userId_grantId_createdAt_idx`(`userId`, `grantId`, `createdAt`),
    INDEX `GrantApplication_decidedAt_idx`(`decidedAt` DESC),
    INDEX `GrantApplication_createdAt_idx`(`createdAt` DESC),
    INDEX `GrantApplication_approvedAmountInUSD_idx`(`approvedAmountInUSD` DESC),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Submission` (
    ...
    INDEX `Submission_listingId_isActive_isArchived_idx`(`listingId`, `isActive`, `isArchived`),
    INDEX `Submission_listingId_userId_isActive_isArchived_idx`(`listingId`, `userId`, `isActive`, `isArchived`),
    INDEX `Submission_listingId_isWinner_isActive_isArchived_idx`(`listingId`, `isWinner`, `isActive`, `isArchived`),
    INDEX `Submission_userId_listingId_createdAt_idx`(`userId`, `listingId`, `createdAt`),
    INDEX `Submission_createdAt_idx`(`createdAt` DESC),
    INDEX `Submission_likeCount_createdAt_idx`(`likeCount` DESC, `createdAt` DESC),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Sponsors` (
    ...
    INDEX `Sponsors_isVerified_st_idx`(`isVerified`, `st`),
    INDEX `Sponsors_isCaution_isVerified_idx`(`isCaution`, `isVerified`),
    INDEX `Sponsors_isActive_isArchived_idx`(`isActive`, `isArchived`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `User` (
    ...
    INDEX `User_lastName_idx`(`lastName`),
...
CREATE TABLE `Scouts` (
    ...
    INDEX `Scouts_listingId_invited_idx`(`listingId`, `invited`),
    INDEX `Scouts_listingId_score_idx`(`listingId`, `score` DESC),
    INDEX `Scouts_userId_listingId_invited_idx`(`userId`, `listingId`, `invited`),
    INDEX `Scouts_score_idx`(`score` DESC),
...
CREATE TABLE `TalentRankings` (
    ...
    INDEX `TalentRankings_skill_timeframe_rank_idx`(`skill`, `timeframe`, `rank` ASC),
    INDEX `TalentRankings_totalEarnedInUSD_idx`(`totalEarnedInUSD` DESC),
    INDEX `TalentRankings_rank_idx`(`rank` ASC),
    INDEX `TalentRankings_winRate_idx`(`winRate` DESC),
...
CREATE TABLE `PoW` (
    ...
    INDEX `PoW_likeCount_createdAt_idx`(`likeCount` DESC, `createdAt` DESC),
    INDEX `PoW_userId_createdAt_idx`(`userId`, `createdAt` DESC),
    INDEX `PoW_userId_id_idx`(`userId`, `id`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `EmailSettings` (
    ...
    INDEX `EmailSettings_category_idx`(`category`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `emailLogs` (
    ...
    `type` ENUM('BOUNTY_REVIEW', 'BOUNTY_DEADLINE', 'BOUNTY_DEADLINE_WEEK', 'BOUNTY_CLOSE_DEADLINE', 'NO_VERIFICATION', 'NO_ACTIVITY', 'NO_REVIEW_SPONSOR_1', 'NO_REVIEW_SPONSOR_2', 'ROLLING_15_DAYS', 'ROLLING_30_DAYS', 'NEW_LISTING', 'ROLLING_UNPUBLISH', 'UNFILLED_PROFILE') NOT NULL,
    `bountyId` VARCHAR(191) NULL,
    `userId` VARCHAR(191) NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,

    INDEX `emailLogs_bountyId_type_idx`(`bountyId`, `type`),
    INDEX `emailLogs_userId_type_idx`(`userId`, `type`),
    INDEX `emailLogs_type_createdAt_idx`(`type`, `createdAt` DESC),
    INDEX `emailLogs_email_type_idx`(`email`, `type`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `ResendLogs` (
    ...
    INDEX `ResendLogs_email_createdAt_idx`(`email`, `createdAt` DESC),
    INDEX `ResendLogs_createdAt_idx`(`createdAt` DESC),
    INDEX `ResendLogs_status_idx`(`status`),
    INDEX `ResendLogs_email_status_idx`(`email`, `status`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `SubscribeBounty` (
    ...
    INDEX `SubscribeBounty_userId_bountyId_idx`(`userId`, `bountyId`),
    INDEX `SubscribeBounty_bountyId_isArchived_idx`(`bountyId`, `isArchived`),
    INDEX `SubscribeBounty_userId_isArchived_idx`(`userId`, `isArchived`),
    UNIQUE INDEX `SubscribeBounty_userId_bountyId_key`(`userId`, `bountyId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
