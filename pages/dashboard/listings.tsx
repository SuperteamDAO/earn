...
function Listing() {
    ...
    const listingData = tanQuery({ queryKey: ['listing', currentSponsor?.orgId || ''], queryFn: ({ queryKey }) => findSponsorListing(queryKey[1]) })
...
const ListingBody = (props: listElm) => {
    ...
    return (
        <Tbody h={"70px"}>
            <SelectWinnerModal selectWinnerOpen={selectWinnerOpen} setSelectWinnerOpen={setSelectWinnerOpen} />
            <Tr>
                ...
                <Td py={"0"}>
                    <Center alignItems={"center"} columnGap="0.9688rem">
                        ...
                        <Text color={"#94A3B8"} fontSize={"0.75rem"} fontWeight={"600"}>{props.deadline}</Text>
                    </Center>
                </Td>
                <Td py={"0"}>
                    <Center>
...
const SelectWinnerModal = ({ selectWinnerOpen, setSelectWinnerOpen }: AppProps) => {
    return (
        <Modal isOpen={selectWinnerOpen} onClose={() => { setSelectWinnerOpen(false) }}>
            <ModalOverlay />
            <ModalContent maxW="33.625rem" borderRadius={"16px"}>
                <Box w={"full"} py={"1.8125rem"} >
                    <Flex px={"1.6875rem"} pb={"1.1875rem"}>
                        <Box height={"2.75rem"} width={"2.75rem"} mt={"0.1rem"} mr={"0.8125rem"}>
                            <Image width="100%"
                        ...
                        <Text ml={"auto"} color={"#A05EBF"} borderRadius={"2.0625rem"} py={"0.3438rem"} px={"0.875rem"} h={"min-content"} fontSize={"0.75rem"} fontWeight={"400"} bg={"rgba(101, 98, 255, 0.14)"} >
                            Bug Bounty
                        </Text>
                    </Flex>
                    ...
                    <Flex px={"1.6875rem"} pt="1.6875rem">
                        <Box w={"full"}>
                            ...
                            <Box mb={"1.3125rem"} mt={"21px"}>
                                ...
                                <Input mt={"0.5rem"} placeholder='Solana Wallet Address'
                    ...
                    <Flex px={"1.6875rem"} flexDir="column">
                        <Button w={"min-content"} leftIcon={<AddIcon />} color={"#94A3B8"} fontSize={"0.6875rem"} variant='link'>
                            ADD WINNER
                        </Button>
                        <Button mt={"1.4375rem"} fontSize={"0.9375rem"} color={"white"} bg={"#6562FF"}>Award Winner</Button>
                    </Flex>
                </Box>
            </ModalContent>
        </Modal >
