...
import { findSponsors, genrateOtp, UpdateUser } from '../../utils/functions';
    ...
    <VStack w={'xl'} gap={10}>
      <VStack mt={20}>
        ...
        <Text
          ...
          textAlign={'center'}
        >
          If you&apos;re ready to start contributing to Solana, you&apos;re in
          the right place.
        </Text>
      </VStack>
      <HStack w="100%">
      ...
      <form style={{ width: '100%' }} onSubmit={handleSubmit(onSubmit)}>
        <FormControl mb={5} w="full" isRequired>
          <VStack gap={2} my={3} align={'start'} mb={'25px'}>
            <Heading color={'gray.400'} fontWeight={600} fontSize={'15px'}>
              Profile Picture
            </Heading>
            <HStack gap={5}>
              <MediaPicker
          ...
          <Box w={'full'} mb={'1.25rem'}>
            <FormLabel color={'gray.400'}>Your One-Line Bio</FormLabel>
            <Textarea
              id={'bio'}
...
const YourWork = ({ setStep }: Step1Props) => {
  ...
  const onSubmit = (data: any) => {
    setpost(true);
    if (
      DropDownValues.skills.length == 0 ||
      DropDownValues.subskills.length == 0 ||
      DropDownValues.interests.length == 0 ||
      DropDownValues.community.length == 0
    ) {
      return false;
    }
    updateState({ ...data, ...DropDownValues });
    setStep((i) => i + 1);
  };
      ...
      <form style={{ width: '100%' }} onSubmit={handleSubmit(onSubmit)}>
        <FormControl mb={5} w="full" isRequired>
          <Flex w={'full'} outline={'0.3125rem'} gap={'1.25rem'} mb={'1.25rem'}>
            ...
            <Box w={'full'}>
              <FormLabel color={'gray.400'}>Work Experience</FormLabel>
              <Input
                id="experience"
                placeholder="Experience in Years"
                type={'number'}
          ...
          <Box w={'full'} mb={'1.25rem'}>
            <FormLabel color={'gray.400'}>Work Preference</FormLabel>
            <Input
              id="workPrefernce"
          ...
          <Box w={'full'} mb={'1.25rem'}>
            <FormLabel color={'gray.400'}>Current Employer</FormLabel>
            <Input
              id="currentEmployer"
          ...
          <Box w={'full'} mb={'1.25rem'}>
            <FormLabel color={'gray.400'}>Community Affiliations</FormLabel>
            <ReactSelect
            ...
            {DropDownValues.community.length == 0 && post && (
              <Text color={'red'}>This field cannot be empty</Text>
            )}
          </Box>
          <Box w={'full'} mb={'1.25rem'}>
            <FormLabel color={'gray.400'}>
          ...
          <Box w={'full'} mb={'1.25rem'}>
            <FormLabel color={'gray.400'}>Your Skills</FormLabel>
            <ReactSelect
              closeMenuOnSelect={false}
              components={animatedComponents}
              isMulti
            ...
            />
            {DropDownValues.skills.length == 0 && post && (
              <Text color={'red'}>This field cannot be empty</Text>
            )}
          </Box>
          <Box w={'full'} mb={'1.25rem'}>
            <FormLabel color={'gray.400'}>Sub Skills</FormLabel>
            <ReactSelect
              closeMenuOnSelect={false}
              components={animatedComponents}
              isMulti
            ...
            />
            {DropDownValues.subskills.length == 0 && post && (
              <Text color={'red'}>This field cannot be empty</Text>
            )}
          </Box>
          <Button
...
const YourLinks = ({
  ...
  const uploadProfile = async (socials: string, pow: string) => {
    ...
    console.log(res);
    const ress = await UpdateUser(userInfo?.id as string, {
      talent: true,
    });
    if (ress) {
      success();
  ...
  const onSubmit = (data: any) => {
    uploadProfile(JSON.stringify(data), JSON.stringify(pow));
  };
        ...
        <form style={{ width: '100%' }} onSubmit={handleSubmit(onSubmit)}>
          <FormControl mb={5} w="full">
            ...
            <Button
              ...
              fontSize={'12px'}
              color={'gray.400'}
              leftIcon={<AddIcon color={'gray.400'} />}
              w={'full'}
              mt="2"
              mb={'6'}
            >
        ...
        <ModalBody>
          <form onSubmit={handleSubmit(onSubmit)}>
            <FormControl isRequired>
              <Box w={'full'} mb={'1.25rem'}>
                <FormLabel color={'gray.400'}>Project Title</FormLabel>
                <Input
                  id="title"
              ...
              <Box w={'full'} mb={'1.25rem'}>
                <FormLabel color={'gray.400'}>Describe your work</FormLabel>
                <Textarea
              ...
              <Box w={'full'} mb={'1.25rem'}>
                <FormLabel color={'gray.400'}>Link</FormLabel>
                <InputGroup>
                  <InputLeftElement
                    pointerEvents="none"
...
const VerifyEmail = ({ setStep }: { setStep: () => void }) => {
  ...
  const otpSend = async () => {
    ...
    const a = await genrateOtp(publicKey?.toBase58() as string, email);
    console.log(a);
  ...
  return (
    <Box w={'xl'} minH={'100vh'}>
      ...
      >
        <Box mt={'4.6875rem'}>
          <FormLabel color={'gray.400'}>Your Email</FormLabel>
          <Input
            required
            type={'email'}
            value={email}
            ...
            w={'34.375rem'}
            placeholder="yb@yashbhardwaj.com"
          />
        </Box>
        <Button
          type="submit"
  ...
  return (
    <Box w={'xl'} minH={'100vh'}>
      ...
      <Flex mt={'130px'} justifyContent="space-between">
        <Text fontSize={'1rem'} color={'gray.400'} fontWeight={'500'}></Text>
        <Text
          onClick={() => {
            otpSend();
          }}
...
const SuccessScreen = () => {
  ...
  return (
    <Box
      ...
      minH={'100vh'}
      h={'100%'}
      backgroundImage="url('/assets/bg/success-bg.png')"
    >
      <VStack>
